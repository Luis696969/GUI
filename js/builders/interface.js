...
export function renderInterfaceDiffusionRows(interfaceEl, summaries) {
  ...
  const previous = Object.fromEntries(Array.from(container.querySelectorAll('.iface-diffusion-entry')).map((row) => [row.dataset.chemical, row.querySelector('.iface-diffusion-value')?.value]).filter(([k, v]) => k && v !== undefined));
  container.innerHTML = '';
  if (!device1 || !device2) { noteEl.textContent = 'Select two devices to display shared chemicals.'; updateCardSummaries(); return; }
  const commonChemicals = device1.chemicals.filter((n) => device2.chemicals.includes(n));
  if (!commonChemicals.length) { noteEl.textContent = 'These devices do not share any chemicals.'; updateCardSummaries(); return; }
  noteEl.textContent = 'Enter one D_interface value for each shared chemical.';
  commonChemicals.forEach((chem) => container.insertAdjacentHTML('beforeend', `<div class="col-md-4 iface-diffusion-entry" data-chemical="${escapeHtml(chem)}"><label class="form-label">${escapeHtml(chem)}</label><input type="number" class="form-control iface-diffusion-value" data-chemical="${escapeHtml(chem)}" min="0" step="any" value="${escapeHtml(previous[chem] ?? '')}"></div>`));
  updateCardSummaries();
}
...
export function refreshInterfaceDeviceOptions(interfaceEl, summaries) {
  ...
  const old1 = select1.value; const old2 = select2.value;
  ...
  select1.innerHTML = options; select2.innerHTML = options;
...
export function buildInterfaces(dom, devices, warnings) {
  ...
  return Array.from(dom.interfacesContainer.querySelectorAll('.interface-entry')).map((el, index) => {
    ...
    const sideLoc1 = buildInterfaceSideLoc(device1, side1, readNumber(el.querySelector('.iface-start1')?.value, `Interface ${index + 1} segment 1 start`), readNumber(el.querySelector('.iface-stop1')?.value, `Interface ${index + 1} segment 1 end`), `Interface ${index + 1}, device 1`);
    const sideLoc2 = buildInterfaceSideLoc(device2, side2, readNumber(el.querySelector('.iface-start2')?.value, `Interface ${index + 1} segment 2 start`), readNumber(el.querySelector('.iface-stop2')?.value, `Interface ${index + 1} segment 2 end`), `Interface ${index + 1}, device 2`);
    ...
    const chemicals1 = deviceById[device1Id].chemicals.map((c) => c.name);
    const chemicals2 = deviceById[device2Id].chemicals.map((c) => c.name);
    ...
    const D_interface = {};
    el.querySelectorAll('.iface-diffusion-value').forEach((input) => { const chemical = normalizeText(input.dataset.chemical); if (chemical) D_interface[chemical] = readNumber(input.value, `Interface ${index + 1} D_interface for ${chemical}`, { min: 0 }); });
    commonChemicals.forEach((chemical) => { if (!(chemical in D_interface)) { D_interface[chemical] = 0; warnings.push(`Interface ${index + 1}: D_interface for ${chemical} was not provided; defaulting to 0.`); } });
    return { device1: device1Id, device2: device2Id, locs: { [device1Id]: sideLoc1.loc, [device2Id]: sideLoc2.loc }, D_interface };
  });
}
